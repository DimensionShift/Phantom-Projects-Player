...
public class PlayerState : MonoBehaviour
{
    ...
    public void IsDead()
    {
        if (health <= 0)
        {
            SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex);   // If the player has run out of health, reload the current level
        }
    }
    ...
    public void ReduceHealth(float h)                                          // Method for reducing the player's health
    {
        SetHealth(health - h);
